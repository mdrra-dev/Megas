...
DESeq2_newDS<-function(vars, set,test, fitType, sfType, reduced, contrast)
{
  ...
  if (is.character(set)) {
    if (length(set) != 1L || is.na(set) || !nzchar(set)) {
      stop("'set' deve essere il nome di un oggetto.", call. = FALSE)
    }
    if (!exists(set, envir = caller_env, inherits = TRUE)) {
      stop(
        sprintf(
          "L'oggetto '%s' non è stato trovato nella sessione server DataSHIELD.",
          set
        ),
        call. = FALSE
      )
    }
    set <- get(set, envir = caller_env, inherits = TRUE)
  }
  #design formula
